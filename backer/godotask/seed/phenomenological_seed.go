	...
	models := []model.OptimizationModel{
		{
			...
			Parameters:        nil, // NULLとして保存
			PerformanceMetric: nil, // NULLとして保存
			IterationCount:    nil, // NULLとして保存
			ConvergenceRate:   nil, // NULLとして保存
		...
		},
		{
			...
			Constraints:       "collision_free, joint_limits, singularity_avoidance",
			Parameters:        nil,
			PerformanceMetric: nil,
			IterationCount:    nil,
			ConvergenceRate:   nil,
			Domain:            "robot_motion",
		...
		},
		{
			...
			Constraints:       "max_force: 100N, stability_margin: 0.1",
			Parameters:        nil,
			PerformanceMetric: nil,
			IterationCount:    nil,
			ConvergenceRate:   nil,
...
func seedRobotArmSpecifications(db *gorm.DB) error {
	...
	log.Printf("✓ Successfully seeded %d robot specifications", len(specs))
	return nil
}