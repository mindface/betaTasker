...
describe('getErrorMessage', () => {
  it('既知のエラーコードに対して正しいメッセージを返す', () => {
    const testCases = [
      { code: ErrorCode.AUTH_INVALID_CREDENTIALS, expected: '認証情報が無効です' },
      { code: ErrorCode.VAL_MISSING_FIELD, expected: '必須項目が入力されていません' },
      { code: ErrorCode.RES_NOT_FOUND, expected: 'リソースが見つかりません' },
      { code: ErrorCode.DB_CONNECTION_FAILED, expected: 'データベース接続に失敗しました' },
      { code: ErrorCode.SYS_INTERNAL_ERROR, expected: '内部エラーが発生しました' },
      { code: ErrorCode.NET_CONNECTION_FAILED, expected: 'ネットワーク接続に失敗しました' },
    ];
