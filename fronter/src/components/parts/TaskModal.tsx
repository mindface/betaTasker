...
const TaskModal = ({
  ...
  const { execute: saveTask } = useApiCall(addTaskClient, {
    onSuccess: () => {
      onClose();
    },
  });
      ...
      <div className="modal-wrapper">
        <form onSubmit={handleSubmit} className="task-form">
          ...
          <div className="form-group p-16">
            {isTask(formData) &&
              formData.heuristics_analysis &&
              formData.heuristics_analysis.length > 0 && (
                    ...
                    {formData.heuristics_analysis.map((analysis) => (
                      <li key={analysis.id}>
                        ...
                        <div className="result p-16">
                          ...
                          <p className="p-b-8">
                            energy_saving: {analysis.result.energy_saving}
                          </p>
                          <p>
                            created_at:{" "}
                            {formatDateTime(
          ...
          <div className="form-group p-16">
            {isTask(formData) &&
              formData.heuristics_patterns &&
              formData.heuristics_patterns.length > 0 && (
                <div className="heuristics-analysis-section">
          ...
          <div className="form-group p-16">
            {isTask(formData) &&
              formData.language_optimizations &&
              formData.language_optimizations.length > 0 && (
                <div className="heuristics-analysis-section">
          ...
          {selectedMemory && (
            <div
              className="selected-memory-info p-8"
              style={{
                margin: "1em 0",
                padding: "0.5em",
                background: "#f6f8fa",
            ...
            >
              <div>
                <b>タイトル:</b> {selectedMemory.title}
              </div>
              <div>
                <b>内容:</b> {selectedMemory.notes}
              </div>
              <div>
                <b>タグ:</b> {selectedMemory.tags}
              </div>
              <div>
                <b>ステータス:</b> {selectedMemory.read_status}
              </div>
            </div>
          )}
