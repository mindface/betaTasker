...
const AssessmentModal = ({
  ...
  const selectedTask = (tasks ?? []).find(
    (t) => t.id === Number(formData?.task_id),
  );
  const relatedMemory = (memories ?? []).find(
    (m) => m.id === selectedTask?.memory_id,
  );
    ...
    >
      <div className="assessment-modal-content">
                ...
                >
                  <div>
                    <span
                      ...
                      onClick={() =>
                        setOpenMemoryId(
                          openMemoryId === memory.id ? null : memory.id,
                        )
                      }
                    >
                      {memory.title}
        ...
        {relatedMemory && (
          ...
          >
            <div>
              <b>関連メモ:</b> {relatedMemory.title}
            </div>
            <div>
              <b>内容:</b> {relatedMemory.notes}
            </div>
            <div>
              <b>タグ:</b> {relatedMemory.tags}
            </div>
            <div>
              <b>ステータス:</b> {relatedMemory.read_status}
            </div>
          </div>
        )}
