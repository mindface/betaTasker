...
const MemoryAidList = ({ code }: Props) => {
  ...
  const { contexts, loading, error } = useAppSelector(
    (state) => state.memoryAid,
  );
  ...
  return (
    <>
      ...
      <CommonModal
        title={`Memory Aids for code: ${code}`}
        isOpen={isModalOpen}
      ...
      >
        <div className="memory-aid-list max-h-m overflow-y-auto">
          <div className="select-box">
            <select
              onChange={(e) => dispatch(loadMemoryAidsByCode(e.target.value))}
            >
            ...
            <div className="card-list p-8">
              {(contexts ?? []).map((ctx) => (
                <div key={ctx.id} className="card-item">
                  <div className="p-b-5 card-title">
                    <b>対象仕事:</b> {ctx.work_target}
                  </div>
                  <div className="p-b-5">
                    <b>機材:</b> {ctx.machine}
                  </div>
                  <div className="p-b-5">
                    <b>材料:</b> {ctx.material_spec}
                  </div>
                  <div className="p-b-5">
                    <b>目的:</b> {ctx.goal}
                  </div>
                  ...
                  <div className="p-b-5">
                    ...
                    <ul>
                      {ctx.technical_factors.map((tf) => (
                        <li className="p-b-5" key={tf.id}>
                          {tf.tool_spec} / {tf.eval_factors}
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    <b>Knowledge Transformations:(知識の変換:)</b>
                    <ul>
                      {ctx.knowledge_transformations.map((kt) => (
                        <li key={kt.id}>
                          {kt.transformation} / {kt.countermeasure}
                        </li>
                      ))}
                    </ul>
                  </div>
