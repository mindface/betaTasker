...
export default function SectionViewer() {
  ...
  const onThree = useCallback(() => {
    function importColliderMeshGroups(
      gltf: GLTF,
      smAnimation: VRMSchema.SecondaryAnimation,
    ): VRMSpringBoneColliderGroup[] {
      ...
      vrmColliderGruops.forEach(async (colliderGroup) => {
        if (
          ...
          return;
        const bone = await gltf.parser.getDependency(
          "node",
          colliderGroup.node,
        );
    ...
    loader.load(
      ...
      async function (gltf) {
        ...
        const vrmExt: VRMSchema.VRM | undefined =
        ...
        const schemaSecondaryAnimation:
          | VRMSchema.SecondaryAnimation
          | undefined = vrmExt?.secondaryAnimation;
        console.log(gltf.parser.extensions?.VRM);
        if (schemaSecondaryAnimation) {
          const colliderGrops = await importColliderMeshGroups(
            gltf,
    ...
    function tick() {
      ...
      if (sendModel.current) {
        sendModel.current.rotation.set(
          left.current,
          top.current,
          botton.current,
        );
