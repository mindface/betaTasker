...
export default function HeuristicsPatterns() {
  const { patterns, loading, error, getPatterns, clearError } =
    useHeuristicsPatterns();
  const { currentModel, modelLoading, modelError, trainModel } =
    useHeuristics();
            ...
            <div className={styles.formRow}>
              <div className={styles.formGroup}>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleTrainFormChange("model_type", e.target.value)
                  }
                  required
                >
                  {modelTypes.map((type) => (
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleTrainFormChange("data_source", e.target.value)
                  }
        ...
        <div className={styles.modelInfo}>
          ...
          <div className={styles.modelCard}>
            <div className={styles.modelHeader}>
              ...
              <span
                className={`${styles.status} ${styles[currentModel.status]}`}
              >
                {currentModel.status}
              </span>
            </div>
            <div className={styles.modelDetails}>
              <div>
                トレーニング日:{" "}
                {new Date(currentModel.trained_at).toLocaleString("ja-JP")}
              </div>
              <div>
                作成日:{" "}
                {new Date(currentModel.created_at).toLocaleString("ja-JP")}
              </div>
            </div>
          </div>
        </div>
            ...
            <div className={styles.stats}>
              ...
              <div className={styles.statCard}>
                ...
                <span>
                  {patterns.length > 0
                    ? (
                        (patterns.reduce((sum, p) => sum + p.accuracy, 0) /
                          patterns.length) *
                        100
                      ).toFixed(1)
                    : 0}
              ...
              {patterns.map((pattern) => (
                <div key={pattern.id} className={styles.patternCard}>
                  ...
                  <div className={styles.patternData}>
                    ...
                    <pre>
                      {JSON.stringify(JSON.parse(pattern.pattern), null, 2)}
                    </pre>
                  </div>
                </div>
              ))}
