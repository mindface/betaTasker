...
export const fetchQualitativeLabelsClient = async () => {
  const data = await fetchApiJsonCore<undefined, QualitativeLabel[]>({
    ...
    method: "GET",
    errorMessage:
      "error fetchQualitativeLabelsClient プロセス最適化一覧取得失敗",
    getKey: "qualitative_labels",
  });
  return data;
};

export const addQualitativeLabelClient = async (
  qualitativeLabel: AddQualitativeLabel,
) => {
...
export const updateQualitativeLabelClient = async (
