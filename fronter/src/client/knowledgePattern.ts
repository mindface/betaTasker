...
export const fetchKnowledgePatternsClient = async () => {
  const data = await fetchApiJsonCore<undefined,KnowledgePattern[]>({
    ...
    method: 'GET',
    errorMessage: 'error fetchKnowledgePatternsClient プロセス最適化一覧取得失敗',
    getKey: 'knowledge_patterns',
  });
  return data;
};

export const addKnowledgePatternClient = async (knowledgePattern: AddKnowledgePattern) => {
...
export const updateKnowledgePatternClient = async (knowledgePattern: KnowledgePattern) => {
