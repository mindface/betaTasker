...
import {
  fetchLanguageOptimizationsClient,
  addLanguageOptimizationClient,
  updateLanguageOptimizationClient,
  deleteLanguageOptimizationClient,
} from "../../client/languageOptimization";
import {
...
export const createLanguageOptimization = createAsyncThunk(
  ...
  async (
    languageOptimizationData: AddLanguageOptimization,
    { rejectWithValue },
  ) => {
    const response = await addLanguageOptimizationClient(
      languageOptimizationData,
    );
    if ("error" in response) {
      return rejectWithValue(response.error);
    }
...
export const updateLanguageOptimization = createAsyncThunk(
  ...
  async (
    languageOptimizationData: LanguageOptimization,
    { rejectWithValue },
  ) => {
    const response = await updateLanguageOptimizationClient(
      languageOptimizationData,
    );
    if ("error" in response) {
      return rejectWithValue(response.error);
...
const languageOptimizationSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        loadLanguageOptimization.fulfilled,
        (state, action: PayloadAction<LanguageOptimization[]>) => {
      ...
      .addCase(
        createLanguageOptimization.fulfilled,
        (state, action: PayloadAction<LanguageOptimization>) => {
          state.languageOptimization.push(action.payload);
        },
      )
      .addCase(
        updateLanguageOptimization.fulfilled,
        (state, action: PayloadAction<LanguageOptimization>) => {
          const idx = state.languageOptimization.findIndex(
      ...
      .addCase(
        removeLanguageOptimization.fulfilled,
        (state, action: PayloadAction<{ id: string }>) => {
          state.languageOptimization = state.languageOptimization.filter(
            (a) => a.id !== action.payload.id,
          );
        },
      );
