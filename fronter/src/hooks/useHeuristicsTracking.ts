...
export function useHeuristicsTracking(
  ...
  console.warn(
    "AAA HeuristicsTracking component is deprecated and may be removed in future versions.",
  );
  ...
  const handleScroll = useCallback(
    debounce((e: Event) => {
      ...
      const scrollDistance = Math.abs(
        element.scrollTop - scrollStartPos.current,
      );
...
export function useGlobalHeuristicsTracking() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "heuristic-applied",
      handleHeuristicApplied as EventListener,
    );
